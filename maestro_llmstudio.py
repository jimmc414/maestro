...
# Define the LMstudio API endpoint and model IDs
LMSTUDIO_API_ENDPOINT = "http://localhost:1234"
ORCHESTRATOR_MODEL = "lmstudio-community/Meta-Llama-3-8B-Instruct-GGUF/Meta-Llama-3-8B-Instruct-Q4_K_M.gguf"
SUB_AGENT_MODEL = "lmstudio-community/Meta-Llama-3-8B-Instruct-GGUF/Meta-Llama-3-8B-Instruct-Q4_K_M.gguf"
REFINER_MODEL = "lmstudio-community/Meta-Llama-3-8B-Instruct-GGUF/Meta-Llama-3-8B-Instruct-Q4_K_M.gguf"

def lmstudio_request(model, messages):
    response = requests.post(
        f"{LMSTUDIO_API_ENDPOINT}/v1/chat/completions",
        json={
            "model": model,
            "messages": messages,
            "max_tokens": 4096
...
def opus_orchestrator(objective, file_content=None, previous_results=None):
    console.print(f"\n[bold]Calling LMstudio Orchestrator for your objective[/bold]")
    ...
    response_text = lmstudio_request(ORCHESTRATOR_MODEL, messages)
    console.print(Panel(response_text, title="[bold green]LMstudio Orchestrator[/bold green]", title_align="left", border_style="green", subtitle="Sending task to LMstudio sub-agent 👇"))
...
def haiku_sub_agent(prompt, previous_haiku_tasks=None, continuation=False):
    ...
    response_text = lmstudio_request(SUB_AGENT_MODEL, messages)
    ...
    console.print(Panel(response_text, title="[bold blue]LMstudio Sub-agent Result[/bold blue]", title_align="left", border_style="blue", subtitle="Task completed, sending result to LMstudio Orchestrator 👇"))
    return response_text

def opus_refine(objective, sub_task_results, filename, projectname, continuation=False):
    console.print("\nCalling LMstudio to provide the refined final output for your objective:")
    ...
    response_text = lmstudio_request(REFINER_MODEL, messages)
